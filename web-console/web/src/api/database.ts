...
export const databaseApi = {
  ...
  updateRow: (schema: string, table: string, rowId: string, values: Record<string, unknown>) => patch<{ ok: boolean; updatedRows: number; schema: string; table: string; message?: string }>(`/api/database/tables/${encodeURIComponent(schema)}/${encodeURIComponent(table)}/row`, { rowId, values }),
