...
import { confirmDialog, confirmSettingsRestart } from "../lib/confirmDialog";
...
import { formatUiSentence, formatResultTitle, formatResultMessage } from "../lib/format";
import { RESTARTABLE_SERVICES, SERVICE_LABELS, VEHICLE_SPAWN_OFFSET_UNITS, UPDATE_RESULT_DISMISS_MS, GAME_UPDATE_TASK_KEY, STACK_UPDATE_TASK_KEY } from "../constants/services";
import { REDBLINK_REPO_URL, REDBLINK_DISCORD_URL, REDBLINK_KOFI_URL } from "../constants/links";
import { PlayerCategoryIconRail } from "../components/PlayerCategoryIconRail";
import type {
  ...
  HomeTaskResult, HomeLoadResult, ConfirmDialogRequest
} from "../types";
...
export function HomePanel({ status, readiness, taskResult, setTaskResult, funcomTokenResult, setFuncomTokenResult, runningAction, setRunningAction, onLoad }: {
  status: string;
  readiness: string;
  ...
  useEffect(() => {
    ...
    async function checkRecentFuncomAuthLogs() {
      ...
      if (authCheck.mismatch) {
        setFuncomTokenResult(persistence.funcomTokenMismatchFromLogResult(authCheck.details || ""));
        return;
      }
      if (authCheck.ok && (persistence.isFuncomTokenAuthFailure(funcomTokenResult) || persistence.hasPersistedFuncomTokenAuthFailure())) {
...
export function PerformanceCards({ performance, error }: { performance: PerformanceSnapshot | null; error: string }) {
  const cards = [
    {
      label: "CPU Usage",
...
export function HomeTaskResultCard({ result }: { result: HomeTaskResult }) {
...
export function ServerPanel(props: {
  ...
  async function saveFuncomToken() {
    ...
    try {
      ...
      const final = await waitForTaskWithUpdates(response.task, (next) => setFuncomTokenResult(persistence.funcomTokenRestartTaskResult(next)));
      const details = taskTechnicalDetails(final);
  ...
  async function loadRestartSchedule() {
    ...
    try {
      ...
      if (values.restart_time && values.restart_time !== "unset") setRestartTime(updatesLib.toHourMinuteTime(values.restart_time));
    } finally {
      setScheduleLoading(false);
    }
  }
  async function saveSchedule() {
    const sanitizedTime = updatesLib.toHourMinuteTime(restartTime);
    if (restartEnabled && !updatesLib.isValidHourMinuteTime(sanitizedTime)) {
...
export function ServicesPanel({ services, setServices, setTask, openLogs, onError }: { services: string; setServices: (text: string) => void; setTask: (task: Task) => void; openLogs: (service: string) => void; onError: (text: string) => void }) {
...
export function AdminToolsPanel({ onError }: { onError: (text: string) => void }) {
      ...
      <div className={`playerAdmin_toggle ${scheduleOpen ? "open" : ""}`}>
        ...
        {scheduleOpen && <div className="playerAdmin_toggleBody">
          ...
          <KeyValueGrid items={[
            ...
            ["Restart Time (Local Server Time)", updatesLib.toHourMinuteTime(restartScheduleValues.restart_time || restartTime)],
          ...
          <div className="action-line schedule-action-line">
            <label className="compact-select">Daily Restart Time<input type="time" step="60" pattern="[0-2][0-9]:[0-5][0-9]" disabled={scheduleSaving} value={restartTime} onChange={(event) => setRestartTime(updatesLib.sanitizeTimeInput(event.target.value))} placeholder="05:00" /></label>
...
export function InlineActionResult({ result, resultKey }: { result: { key: string; tone: "success" | "danger" | "neutral"; text: string; pending?: boolean } | null; resultKey: string }) {
...
export function PlayersPanel({ setTask, onError }: { setTask: (task: Task) => void; onError: (text: string) => void }) {
...
export function PlayerActions({ dbPlayerId, actionPlayerId, playerName, setTask, onError, onRefresh }: { dbPlayerId: string; actionPlayerId: string; playerName: string; setTask: (task: Task) => void; onError: (text: string) => void; onRefresh: () => void }) {
  const [itemName, setItemName] = useState("");
  const [itemId, setItemId] = useState("");
  const [selectedItem, setSelectedItem] = useState<CatalogItem | null>(null);
...
async function pollDatabasePasswordRestart(
  ...
  try {
    ...
    for (let i = 0; i < 3600 && !isTerminalTask(current.status); i += 1) {
      ...
      persistence.persistDatabasePasswordState(runningState);
    ...
    persistence.persistDatabasePasswordState(failed);
    setState(failed);
    return;
  }
  ...
  persistence.persistDatabasePasswordState(next);
  setState(next);
...
async function waitForFuncomTokenRestartValidation(
  ...
  for (let attempt = 0; attempt < 72; attempt += 1) {
    ...
    if (authCheck.mismatch) return persistence.funcomTokenMismatchResult([details, authCheck.details || ""].filter(Boolean).join("\n"));
    ...
    if (status && isHomeActionComplete(status.statusText, status.readinessText)) {
      ...
      if (finalCheck.mismatch) return persistence.funcomTokenMismatchResult([details, finalCheck.details || ""].filter(Boolean).join("\n"));
      return {
        status: "succeeded",
...
export function GameUpdateProgress({ task, repairTask, onRetry, onFixSteamcmd }: { task: Task; repairTask: Task | null; onRetry: () => Promise<void>; onFixSteamcmd: () => Promise<void> }) {
  const progress = updatesLib.summarizeGameUpdateProgress(task);
  ...
  const repairable = task.status === "failed" && updatesLib.isSteamcmdManifestFailure(task);
...
export function StackUpdateProgress({ task, onRetry }: { task: Task; onRetry: () => Promise<void> }) {
  const progress = updatesLib.summarizeStackUpdateProgress(task);
  const running = !isTerminalTask(task.status);
...
export function PlayerDetailTab({ playerId, tab, onError, onActionLog }: { playerId: string; tab: string; onError: (text: string) => void; onActionLog?: (actionType: string, target: string, amount: string, notes: string) => void }) {
  const [data, setData] = useState<Record<string, unknown> | null>(null);
  const [message, setMessage] = useState("");
  const [messageDetails, setMessageDetails] = useState("");
  async function loadTab() {
    const unsupportedFeatures = new Set(["progression", "events", "stats", "history"]);
    if (unsupportedFeatures.has(tab)) {
      setData({ capabilities: { [tab]: false }, rows: [], reason: `${tab} is not available for this database schema yet.` });
      setMessage("");
      return;
    }
...
export function LogsPanel({ selectedService, setSelectedService, text, setText, onError }: { selectedService: string; setSelectedService: (service: string) => void; text: string; setText: Dispatch<SetStateAction<string>>; onError: (text: string) => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([sietchesApi.sietchDimensions("Survival_1"), sietchesApi.sietchDimensions("Survival_1", true)])
...
export function DatabasePanel() {
  ...
  const [databasePasswordState, setDatabasePasswordState] = useState<DatabasePasswordState>(() => persistence.loadDatabasePasswordState());
...
export function databasePreviewColumns(preview: { columns?: { name: string }[] } | null) {
  return (preview?.columns || []).map((column) => column.name).filter((name) => name !== "__rowid");
}

export function omitInternalRowFields(row: Record<string, unknown>) {
...
export function CarePackagePanel({ onError }: { onError: (text: string) => void }) {
...
export function normalizeCarePackageConfig(config: CarePackageConfig): CarePackageConfig {
...
export function normalizeCarePackageSendMessage(value: unknown) {
  const text = typeof value === "string" ? value : "";
  return text.trim() === "Welcome to the server" ? "" : text;
}

export function displayCarePackageConfig(config: CarePackageConfig) {
...
export function formatCarePackageError(value: string) {
...
export function carePackageActiveKit(config: CarePackageConfig) {
  return config.kits.find((kit) => kit.id === config.activeKitId) || config.kits[0] || { id: "", name: "", items: [], xp: 0 };
}

export function carePackageGrantSummary(kit?: CarePackageEntry) {
...
export function carePackageRuleName(rule: CarePackageAutoGrantRule | undefined, kits: CarePackageEntry[]) {
  const kit = kits.find((entry) => entry.id === rule?.kitId);
  return kit?.name ? `${kit.name} rule` : "Auto Grant rule";
}

export function carePackageConditionLabel(rule: CarePackageAutoGrantRule) {
  if (rule.grantWhen === "last_seen") return `Last Seen ${Number(rule.lastSeenDays) || 30} Days Ago`;
  return "First Online";
}

export function uniqueCarePackageId(kits: CarePackageEntry[], base: string) {
...
export function uniquePackageRuleId(rules: CarePackageAutoGrantRule[], base: string) {
...
export function carePackageEligibleRows(rows: Record<string, unknown>[]) {
  return rows.map((row) => ({
...
export function carePackageHistoryRows(rows: Record<string, unknown>[]) {
  return rows.map((row) => ({
    ...row,
...
export function carePackageHistoryPackageName(row: Record<string, unknown>) {
  return String(row.kitName || row.packageName || row.version || "Selected package");
}

export function carePackageHistoryPlayerName(row: Record<string, unknown>) {
  return String(row.character_name || row.playerName || row.action_player_id || row.playerId || "Selected player");
}

export function CarePackageResult({ output, technicalOutput }: { output: string; technicalOutput: string }) {
...
export function friendlyCarePackageResultLine(line: string) {
  return line
...
export function formatCarePackageGrantResult(result: Record<string, unknown>) {
...
export function describeCarePackageAction(action: Record<string, unknown>) {
...
export function StoragePanel({ onError }: { onError: (text: string) => void }) {
  const [rows, setRows] = useState<Record<string, unknown>[]>([]);
  const [selected, setSelected] = useState<Record<string, unknown> | null>(null);
...
export function BackupsPanel({ backupRestoreTask, setBackupRestoreTask, onError }: { backupRestoreTask: Task | null; setBackupRestoreTask: (task: Task | null) => void; onError: (text: string) => void }) {
  const [rows, setRows] = useState<Record<string, unknown>[]>([]);
  ...
  async function refreshAutoBackup() {
    ...
    if (status.backupTime) setAutoTime(updatesLib.toHourMinuteTime(status.backupTime));
  ...
  async function saveAutomaticBackups() {
    const sanitizedTime = updatesLib.toHourMinuteTime(autoTime);
    if (autoEnabled && !updatesLib.isValidHourMinuteTime(sanitizedTime)) {
    ...
    <section className="panel">
      ...
      <section className="action-section">
        ...
        <KeyValueGrid items={[
          ...
          ["Backup Time (Local Server Time)", updatesLib.toHourMinuteTime(autoStatus.backupTime || autoTime)],
        ...
        <div className="action-line backup-auto-controls">
          <label className="compact-select">Daily Backup Time<input type="time" step="60" pattern="[0-2][0-9]:[0-5][0-9]" value={autoTime} onChange={(event) => setAutoTime(updatesLib.sanitizeTimeInput(event.target.value))} placeholder="05:00" /></label>
...
export function BackupResultCard({ result }: { result: BackupResult }) {
...
export function backupRestoreTaskResult(task: Task): BackupResult {
...
export function backupRestoreStageMessage(task: Task) {
...
export function summarizeBackupTask(task: Task, successTitle: string, failureTitle: string): BackupResult {
  const details = task.logLines.map((line) => line.line).join("\n");
  if (task.status === "succeeded") {
...
export function extractBackupName(text: string) {
...
export function LiveMapPanel({ onError }: { onError: (text: string) => void }) {
...
export function MapsPanel({ setTask, onError }: { setTask: (task: Task) => void; onError: (text: string) => void }) {
  ...
  const [mapsResult, setMapsResult] = useState<HomeTaskResult | null>(() => persistence.loadPersistedMapsResult());
  ...
  async function runTaskAndRefresh(action: () => Promise<{ task: Task }>, runningTitle = "Applying Map Changes", successTitle = "Map Changes Applied") {
    ...
    persistence.persistMapsTask({ taskId: response.task.id, result: started, runningTitle, successTitle });
    const final = await waitForTaskWithUpdates(response.task, (task) => {
      ...
      persistence.persistMapsTask({ taskId: task.id, result: nextProgress, runningTitle, successTitle });
    });
    ...
    setMapsResult(next);
    persistence.persistMapsTask(null);
    await loadMaps();
  ...
  async function runTaskSequenceAndRefresh(actions: Array<{ label: string; run: () => Promise<{ task: Task }> }>, runningTitle = "Applying Map Changes", successTitle = "Map Changes Applied", options: { saveAcceptedMessage?: string } = {}) {
    ...
    persistence.persistMapsTask({ result: { status: "running", title: runningTitle, message: savingMessage }, runningTitle, successTitle });
    ...
    for (const [index, action] of actions.entries()) {
      ...
      if (!handedOffToWarming) {
        ...
        persistence.persistMapsTask({ result: { status: "running", title: runningTitle, message: progressMessage }, runningTitle, successTitle });
      }
      const response = await action.run();
      if (!handedOffToWarming) {
        persistence.persistMapsTask({ taskId: response.task.id, result: { status: "running", title: runningTitle, message: progressMessage }, runningTitle, successTitle });
      }
  ...
  async function loadSietches(options: { preserveDrafts?: boolean } = {}) {
    const [list, dimensions, ids] = await Promise.all([sietchesApi.sietches(), sietchesApi.sietchDimensions("Survival_1"), sietchesApi.sietchDimensions("Survival_1", true)]);
  ...
  useEffect(() => {
    const persisted = persistence.loadPersistedMapsTask();
    ...
    (async () => {
      ...
      while (!cancelled && !isTerminalTask(current.status)) {
        ...
        setMapsResult(nextProgress);
        persistence.persistMapsTask({ taskId: current.id, result: nextProgress, runningTitle, successTitle });
        await new Promise((resolvePromise) => window.setTimeout(resolvePromise, 1000));
      ...
      if (persistence.isMissingPersistedTaskError(error)) {
        persistence.persistMapsTask(null);
        setMapsResult(null);
        return;
      }
    ...
    if (includeActive && activeSietches && activeSietchesDirty) {
      ...
      if (requestedActive > survivalSietchRows.length) {
        actions.push({
          ...
          run: () => sietchesApi.updateSietches({ action: "set-max", map: "Survival_1", count: requestedActive, confirmation: "UPDATE SIETCHES" })
        });
      }
      activeAction = {
        ...
        run: () => sietchesApi.updateSietches({ action: "set-active", map: "Survival_1", count: requestedActive, confirmation: "UPDATE SIETCHES" })
      };
    }
      ...
      for (const sietch of survivalSietchRows) {
        ...
        if (nameChanged && passwordChanged) {
          actions.push({
            ...
            run: () => sietchesApi.updateSietches({ action: "set-settings", partitionId: sietch.partitionId, displayName: draft.displayName, password: draft.password, confirmation: "UPDATE SIETCHES" })
          });
          continue;
        }
        if (nameChanged) {
          actions.push({
            ...
            run: () => sietchesApi.updateSietches({ action: "set-display", partitionId: sietch.partitionId, displayName: draft.displayName, confirmation: "UPDATE SIETCHES" })
          });
        }
        if (passwordChanged) {
          actions.push({
            label: `Saving password for ${targetName}`,
            run: () => sietchesApi.updateSietches({ action: "set-password", partitionId: sietch.partitionId, password: draft.password, confirmation: "UPDATE SIETCHES" })
          });
        }
      }
    }
  ...
  async function enableDualDeepDesert() {
    ...
    await runTaskAndRefresh(
      () => sietchesApi.updateDeepdesert({ action: "enable", confirmation: "UPDATE DEEP DESERT" }),
  ...
  async function disableDualDeepDesert(row?: Record<string, unknown>) {
    ...
    await runTaskAndRefresh(
      () => sietchesApi.updateDeepdesert({ action: "disable", confirmation: "UPDATE DEEP DESERT" }),
...
export function SettingsEditor({ fields, values, onChange }: { fields: UserSettingField[]; values: Record<string, string>; onChange: (id: string, value: string) => void }) {
...
export function SettingsCardGrid({ fields, values, onChange }: { fields: UserSettingField[]; values: Record<string, string>; onChange: (id: string, value: string) => void }) {
...
export function SettingControl({ field, value, onChange }: { field: UserSettingField; value: string; onChange: (value: string) => void }) {
...
export function MemoryUsageBar({ row, fallback, configuredLimit }: { row: LiveMapMemoryRow | null; fallback: string; configuredLimit?: unknown }) {
...
export function ConfiguredMemoryValue({ value }: { value: unknown }) {
...
export function sietchHasPassword(row: SietchRow | null | undefined, draft?: { password: string }) {
  return Boolean(row?.passwordSet || row?.password || (draft?.password && draft.password !== SIETCH_PASSWORD_MASK));
}

export function sietchPasswordInputValue(row: SietchRow, draft: { password: string }, touched: boolean) {
  if (touched) return draft.password;
  return row.passwordSet ? SIETCH_PASSWORD_MASK : draft.password;
}

export function defaultSietchName(row: SietchRow) {
...
export function sietchTargetDisplayName(row: SietchRow, draftDisplayName?: string) {
  const draft = String(draftDisplayName ?? "").trim();
  if (draft) return draft;
  return defaultSietchName(row) || row.displayName || `partition ${row.partitionId}`;
}

export function SietchMapName({ name, sietch, draft }: { name: string; sietch?: SietchRow | null; draft?: { password: string } }) {
...
export function SietchName({ sietch, draft }: { sietch: SietchRow; draft?: { password: string } }) {
...
export function passwordPlaceholder(passwordSet: boolean) {
  return "Empty for none";
}

export function groupSettingsFields(fields: UserSettingField[]): [string, UserSettingField[]][] {
...
export function settingsCategory(value: string) {
...
export function friendlySettingLabel(id: string, fallback: string) {
...
export function parseUserSettingsMap(text: string) {
  return Object.fromEntries(parseUserSettingRows(text).map((row) => [String(row.key || row.setting), String(row.value ?? "")]));
}

export function changedKeys(original: Record<string, string>, draft: Record<string, string>, keys: string[]) {
...
export function memoryForMap(rows: LiveMapMemoryRow[], map: string, row?: Record<string, unknown>) {
...
export function partitionMemoryValue(memoryText: string, partitionId: string, fallback: string, mapName = "Survival_1") {
...
export function deepDesertPartitionName(row: Record<string, unknown>) {
...
export function settingsTargetKey(map: string, partitionId = "") {
  return `${map}::${partitionId}`;
}

export function buildUserGameTargets(
...
export function liveMemoryFallback(row: Record<string, unknown>) {
...
export function countMarkers(markers: LiveMapMarker[]) {
...
export function friendlyMarkerName(marker: LiveMapMarker) {
...
export function friendlyMarkerType(type: string) {
  return {
    player: "Player",
    vehicle: "Vehicle",
    base: "Base",
...
export function liveMapPlayerStatus(marker: LiveMapMarker) {
...
export function UpdatesPanel({ setTask }: { setTask: (task: Task) => void }) {
  const [gameUpdateTask, setGameUpdateTask] = useState<Task | null>(() => persistence.loadPersistedUpdateTask(GAME_UPDATE_TASK_KEY));
  const [stackUpdateTask, setStackUpdateTask] = useState<Task | null>(() => persistence.loadPersistedUpdateTask(STACK_UPDATE_TASK_KEY));
  ...
  async function checkGame() {
    ...
    setGameStatus(persistence.parseUpdateTask(final));
  }
  ...
  async function checkStack() {
    ...
    setStackStatus(persistence.parseUpdateTask(final));
  }
  ...
  async function applyGameUpdate() {
    ...
    persistence.persistUpdateTask(GAME_UPDATE_TASK_KEY, response.task);
  ...
  async function applyStackUpdate() {
    ...
    persistence.persistUpdateTask(STACK_UPDATE_TASK_KEY, response.task);
  ...
  async function loadAutoGame() {
    try {
      ...
      if (values.auto_update_time) setAutoGameTime(updatesLib.toHourMinuteTime(values.auto_update_time));
    } finally {
      setAutoGameLoading(false);
    }
  }
  async function saveAutoGame() {
    const sanitizedTime = updatesLib.toHourMinuteTime(autoGameTime);
    if (autoGameEnabled && !updatesLib.isValidHourMinuteTime(sanitizedTime)) {
  ...
  useEffect(() => {
    if (!gameUpdateTask || isTerminalTask(gameUpdateTask.status)) {
      persistence.persistUpdateTask(GAME_UPDATE_TASK_KEY, gameUpdateTask);
      return;
    }
    let cancelled = false;
    persistence.persistUpdateTask(GAME_UPDATE_TASK_KEY, gameUpdateTask);
    ...
    void (async () => {
      ...
      for (let i = 0; i < 3600 && !cancelled && !isTerminalTask(current.status); i += 1) {
        ...
        persistence.persistUpdateTask(GAME_UPDATE_TASK_KEY, current);
      }
      if (!cancelled && current.status === "succeeded") refreshGameStatus();
    })().catch(() => {
      if (cancelled) return;
      persistence.persistUpdateTask(GAME_UPDATE_TASK_KEY, null);
  ...
  useEffect(() => {
    if (!stackUpdateTask || isTerminalTask(stackUpdateTask.status)) {
      persistence.persistUpdateTask(STACK_UPDATE_TASK_KEY, stackUpdateTask);
      return;
    }
    let cancelled = false;
    persistence.persistUpdateTask(STACK_UPDATE_TASK_KEY, stackUpdateTask);
    ...
    void (async () => {
      ...
      for (let i = 0; i < 3600 && !cancelled && !isTerminalTask(current.status); i += 1) {
        ...
        persistence.persistUpdateTask(STACK_UPDATE_TASK_KEY, current);
      }
      if (!cancelled && current.status === "succeeded") refreshStackStatus();
    })().catch(() => {
      if (cancelled) return;
      persistence.persistUpdateTask(STACK_UPDATE_TASK_KEY, null);
  ...
  const gameCanApply = updatesLib.canApplyUpdateStatus(gameStatus) && !gameUpdateRunning;
  const stackUpdateRunning = Boolean(stackUpdateTask && !isTerminalTask(stackUpdateTask.status));
  const stackCanApply = updatesLib.canApplyUpdateStatus(stackStatus) && !stackUpdateRunning;
  return <section className="panel">
    <h2>Updates</h2>
    <div className="action-sections">
      <section className="action-section">
        <div className="panel-title"><h4>Game Update</h4><StatusPill value={gameStatus.status} /></div>
        <KeyValueGrid items={[["Current Build", updatesLib.updateDisplayValue(gameStatus, "current")], ["Latest Build", updatesLib.updateDisplayValue(gameStatus, "latest")], ["Status", gameStatus.status]]} />
      ...
      <section className="action-section">
        ...
        <KeyValueGrid items={[["Current Console Version", updatesLib.updateDisplayValue(stackStatus, "current", updatesLib.formatStackVersionLabel)], ["Latest Console Version", updatesLib.updateDisplayValue(stackStatus, "latest", updatesLib.formatStackVersionLabel)], ["Status", stackStatus.status]]} />
      ...
      <section className="action-section">
        ...
        <KeyValueGrid items={[
          ...
          ["Check Time (Local Server Time)", updatesLib.toHourMinuteTime(autoGameValues.auto_update_time || autoGameTime)],
        ...
        <div className="action-line schedule-action-line auto-game-action-line">
          <label className="compact-select">Daily Check Time<input type="time" step="60" pattern="[0-2][0-9]:[0-5][0-9]" disabled={autoGameSaving} value={autoGameTime} onChange={(event) => setAutoGameTime(updatesLib.sanitizeTimeInput(event.target.value))} placeholder="05:00" /></label>
...
export function SettingsPanel({ onPasswordChanged }: { onPasswordChanged: () => Promise<void> }) {
  ...
  async function changeLoginPassword() {
    ...
    try {
      await settingsApi.changeAdminPassword(currentPassword, newPassword);
      setCurrentPassword("");
      setNewPassword("");
      setConfirmPassword("");
...
export function adminPasswordChecks(password: string) {
  return [
...
export function HomeHealthCards({ status, readiness, readinessWarning, loading, runningAction, taskResult, funcomTokenResult }: { status: string; readiness: string; readinessWarning: string; loading: boolean; runningAction: "start" | "stop" | "restart" | ""; taskResult: HomeTaskResult | null; funcomTokenResult: HomeTaskResult | null }) {
  const funcomTokenCheckRunning = funcomTokenResult?.status === "running";
  const summary = summarizeHomeStatus(status, readiness, readinessWarning, loading, runningAction, taskResult, !funcomTokenCheckRunning && (persistence.isFuncomTokenAuthFailure(funcomTokenResult) || persistence.hasPersistedFuncomTokenAuthFailure()));
...
export function DoctorSummary({ text, readiness }: { text: string; readiness: string }) {
...
export function doctorAdvice(issue: string) {
...
export function PlayerSummary({ detail, fallback, dbPlayerId, actionPlayerId, actions }: { detail: Record<string, unknown> | null; fallback: Record<string, unknown>; dbPlayerId: string; actionPlayerId: string; actions?: React.ReactNode }) {
...
export function PlayerCapabilities({ capabilities }: { capabilities: Record<string, unknown> }) {
...
export function RuntimeSettingsSummary({ settings }: { settings: Record<string, unknown> | null }) {
  const config = (settings?.config as Record<string, unknown> | undefined) || {};
...
export function MapCommandSummary({ text }: { text: string }) {
...
export function parseMemoryRows(text: string): Record<string, unknown>[] {
...
export function parseServerPartitionRows(text: string): Record<string, unknown>[] {
...
export function parseReadinessPartitionStatuses(text: string) {
...
export function mergeMapAndMemoryRows(mapsText: string, memoryText: string, serversText = ""): Record<string, unknown>[] {
...
export function mapRuntimeStatus(row: { assignedServer?: unknown; ready?: unknown; alive?: unknown }) {
...
export function mapCanForceDespawn(row: Record<string, unknown>) {
  return /^(Warming|Running)$/i.test(String(row.status || "").trim());
}

export function strongestMapStatus(a: string, b: string) {
  const order = ["Not Available", "Not Running", "Warming", "Running"];
  return order.indexOf(b) > order.indexOf(a) ? b : a || b;
}

export function friendlyMapMode(value: string) {
...
export function modeInputValue(value: string) {
...
export function memoryInputValue(value: string) {
  const match = String(value || "").match(/(\d+(?:\.\d+)?)\s*(GB|GiB?|MB|MiB?|[gGmM])?/);
  if (!match) return "8";
  return match[1];
}

export function memoryValueToBytes(value: string) {
...
export function commandStatusSummary(result: { stdout?: string; stderr?: string; exitCode?: number } | null) {
...
export function parseUserSettingRows(text: string) {
  return stripAnsi(text).split(/\r?\n/).map((line) => {
...
export function friendlySettingName(key: string) {
...
export function friendlyMapName(value: unknown) {
  const text = String(value || "");
...
export function friendlyFileLabel(value: string) {
  return {
...
export function friendlyTabName(value: string) {
  return {
    inventory: "Inventory",
    currency: "Currency",
    factions: "Factions",
...
export function formatTechnicalText(sections: [string, string][]) {
  return sections.map(([title, text]) => `# ${title}\n${text}`).join("\n\n");
}

export function summarizeHomeStatus(status: string, readiness: string, readinessWarning: string, loading: boolean, runningAction: "start" | "stop" | "restart" | "" = "", taskResult: HomeTaskResult | null = null, funcomTokenAuthFailure = false) {
...
export function isHomeCoreReadyWithReview(
  status: string,
  readiness: string,
...
export function isReadyHomeCard(item: { label: string; status: string; detail: string }) {
...
export function isHomeReadinessOperational(readiness: string) {
...
export function attentionHomeHealthCards() {
...
export function transitionHomeHealthCard(item: { label: string; status: string; detail: string }, runningAction: "start" | "stop" | "restart" | "") {
...
export function preferKnownHomeHealth(primary: { label: string; status: string; detail: string }, fallback: { label: string; status: string; detail: string }) {
...
export function isHomeBootStarting(status: string, readiness: string) {
  const text = `${status}\n${readiness}`;
...
export function friendlyHomeOverall(value: string) {
...
export function isMapRuntimeHandoffTask(task: Task) {
  const text = [
...
export function friendlyHomeStatusError(error: string) {
...
export function conciseTaskMessage(text: string) {
...
export function summarizeListeners(text: string) {
...
export function summarizeDatabase(text: string) {
...
export function summarizeGameServers(text: string) {
...
export function summarizeRabbit(text: string) {
...
export function summarizeFls(text: string) {
...
export function summarizeReadinessContainers(text: string) {
...
export function summarizeReadinessListeners(text: string) {
...
export function summarizeReadinessDatabase(text: string) {
...
export function summarizeReadinessGameServers(text: string) {
...
export function summarizeReadinessRabbit(text: string) {
...
export function summarizeReadinessFls(text: string) {
...
export function readinessRows(text: string, pattern: RegExp) {
  return stripAnsi(text).split(/\r?\n/).map((line) => line.trim()).filter((line) => /^(OK|WARN|FAIL)\s+/i.test(line) && pattern.test(line));
}

export function numberAfterLabel(lines: string[], label: string) {
...
export function findPopulation(text: string) {
  for (const rawLine of stripAnsi(text).split(/\r?\n/)) {
    const line = rawLine.trim();
...
export function normalizePopulationPair(current: string, max: string) {
...
export function formatHomePopulation(value: string) {
...
export function findLineValue(text: string, keys: string[]) {
  for (const rawLine of stripAnsi(text).split(/\r?\n/)) {
...
export function summarizeSection(text: string, section: string) {
...
export function sectionLines(text: string, section: string) {
...
export function summarizeSubsystem(text: string, keywords: string[]) {
...
export function inferStatus(text: string) {
...
export function formatMutationResult(result: unknown) {
...
export function parseJsonMaybe(text: string) {
...
export function formatTimerStatus(value: string) {
  const text = String(value || "").trim();
...
export function friendlyIssueLine(line: string) {
  return line
    .replace(/^OK\s+/i, "")
    .replace(/^WARN\s+/i, "")
    .replace(/^WAIT\s+/i, "")
...
export function parseCatalogItems(text: string): CatalogItem[] {
...
export function parseCatalogRows(text: string): Record<string, unknown>[] {
...
export function splitTemplateList(text: string) {
...
export function parseServiceRows(text: string) {
...
export function parseBackupRows(text: string) {
  return text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
...
export function friendlyBackupType(name: string, line: string) {
...
export function parseHistoryRows(text: string, players: Record<string, unknown>[] = [], scope: "all" | "admin-tools" = "all") {
...
export function adminHistoryLineMatchesScope(command: string, target: string, scope: "all" | "admin-tools") {
...
export function formatAdminHistoryTime(value: string) {
...
export function friendlyAdminHistoryValue(value: string) {
  const text = String(value || "-").replace(/^web[-_]/i, "").replace(/[-_]+/g, " ").replace(/\s+/g, " ").trim();
  if (!text || text === "-") return "-";
  return titleCaseWords(text);
}

export function friendlyAdminHistoryAction(value: string) {
  const raw = String(value || "").trim();
  const labels: Record<string, string> = {
...
export function friendlyAdminHistoryTarget(value: string, players: Record<string, unknown>[]) {
  const text = String(value || "-").trim();
  if (!text || text === "-") return "-";
...
export function adminHistoryTargetCandidates(player: Record<string, unknown>) {
  return [
...
export function matchesAdminHistoryTarget(candidate: string, target: string) {
...
export function friendlyAdminHistorySummary(friendly: string, path: string, payload: string, command = "") {
...
export function friendlyLogServiceName(name: string, sietches: SietchRow[] = []) {
...
export function survivalLogPartitionId(name: string) {
...
export function isServiceReady(status: string, service: string) {
...
export function serviceContainerName(service: string) {
...
export function serviceActionName(name: string, action: "logs" | "restart") {
